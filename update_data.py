...
def build_location(ev, budget, cache):
    ...
    # Case 3: normal "street + cross street(s)" intersection
    if street and cross_parts:
        ...
        # None of the cross-street candidates matched (mangled abbreviation,
        # or Nominatim just doesn't have it) — fall back to the main street.
        cross = cross_parts[0]
        fallback_key = f"sb:{street.lower()}|{borough.lower()}"
        geo = geocode_query(cache, fallback_key, {"q": f"{street}, {borough_part}Toronto, Ontario, Canada"}, budget)
        if geo:
            return geo, f"{loc_label}{street} (near {cross})", True
...
def fetch_fire():
    ...
    out = []
    raw_events = fetch_fire_events()
    ungeocoded = 0
    for ev in raw_events:
        geo, location, approx = build_location(ev, budget, cache)
        if not geo:
            ungeocoded += 1
            print(f"    [fire geocode miss] prime={ev['prime_street']!r} cross={ev['cross_streets']!r}", file=sys.stderr)
    ...
    if raw_events and ungeocoded == len(raw_events):
        raise RuntimeError(
            f"XML returned {len(raw_events)} event(s) but 0 geocoded — "
            f"likely geocode budget exhausted or Nominatim failing, not a genuine quiet period"
        )
    print(f"  [fire] {len(raw_events)} raw events, {ungeocoded} not yet geocoded, {len(out)} usable")
    return out
